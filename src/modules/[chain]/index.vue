...
const tickerUsd = computed(() => Number(ticker.value?.converted_last?.usd || 0));
...
function shortName(name?: string, id?: string) {
  ...
  return value.toLowerCase().startsWith("ibc/") || value.toLowerCase().startsWith("0x") ? (id || value) : value;
}
...
const amount = computed({
  get: () => {
    return quantity.value * tickerUsd.value || 0;
  },
  set: (val) => {
    quantity.value = tickerUsd.value ? val / tickerUsd.value : 0;
  },
});
</script>
          ...
          <div>
            ...
            <div class="flex">
              ...
              <a
                ...
                :href="blockchain.chainName === 'xitcoin-testnet' ? 'https://faucet-testnet.xitcoin.org/' : tickerUrl(ticker?.trade_url)"
                target="_blank"
              >
                {{ blockchain.chainName === 'xitcoin-testnet' ? 'Get test XTC' : `${$t('index.buy')} ${coinInfo.symbol || ''}` }}
              </a>
            </div>
          </div>
        </div>
